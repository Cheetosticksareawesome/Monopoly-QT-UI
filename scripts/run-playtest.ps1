...
try {
    ...
    $testArgs += @("-I$qt/include", "-I$qt/include/QtWidgets", "-I$qt/include/QtGui", "-I$qt/include/QtCore", 'scripts/pawn-playtest.cpp')
    ...
    $testArgs += @("-L$qt/lib", '-lQt6Widgets', '-lQt6Gui', '-lQt6Core', '-o', 'build/pawn-playtest.exe')
    & $compiler @testArgs
    if ($LASTEXITCODE -ne 0) { throw 'Test runner compilation failed.' }

    if (!$BuildOnly) {
        $env:QT_QPA_PLATFORM = 'offscreen'
        Write-Host 'Running up to 100,000 rolls. Output: build/playtest-run.md'
        & './build/pawn-playtest.exe'
